...
def allure_story(title: str, url: str = None, name: str = "Ссылка на сторю в Jira", parametrized_func: bool = True):
    ...
    - объединять таким декоратором параметризованные и не параметризованные тестовые функции нельзя!
    - при этом отсутствует необходимость в декораторе `@allure_testcase`
    ---
